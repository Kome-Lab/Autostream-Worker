...
- 描画した映像をjob-scoped暗号化SRTで選択済みEncoder/Recorderへ送り、診断用eventも転送します。
...
`/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc` so existing
`worker.env` files keep the same font selection. If the selected file is missing
...
`/opt/autostream/local-executor/ports/worker.json`; its `bind_address` is the
only local listener authority and its positive `config_revision` is reported by
`/updater/version`. The JSON also carries `schema_version: 2` and
`service_type: worker`. Missing, malformed, writable, or mismatched credentials
fail closed. There is no bind or revision environment fallback.
...
`AUTOSTREAM_NODE_CONFIG` には Control Panel の Node登録で生成した `config.yml` を指定します。Node Runtime Token、service identity、public endpoint、listener credential selector、stream-scoped token の検証に使う `stream_ingest.signing_key` はこのファイルから読み込みます。WorkerのsignalsはControl Panelのtyped Observability経路へ送信します。

Encoder/Recorder のevent送信先とworker-event tokenはControl Panelのstream job contextで `encoder_recorder_url` / `stream_ingest_token` として渡されます。映像用のSRT URL・passphrase・key lengthも同じstart処理内で一度だけ渡され、URLやargv、log、statusへ秘密を埋め込みません。
