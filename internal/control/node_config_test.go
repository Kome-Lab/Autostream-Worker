...
func TestConfigFromEnvUsesNodeConfig(t *testing.T) {
	...
	cfg := ConfigFromEnv()
	if cfg.ControlPanelURL != "https://panel.example.jp" || cfg.Token != "runtime-secret" || cfg.ServiceID != "worker-01" || cfg.ServiceName != "Worker 01" || cfg.ServicePublicURL != "https://worker.example.jp:8443" || cfg.BindAddress != "127.0.0.1:18084" || cfg.ConfigRevision != 27 {
...
func writeNodeConfigForTest(t *testing.T, nodeType string) string {
	...
	credential := `{"schema_version":2,"service_type":"` + nodeType + `","bind_address":"127.0.0.1:18084","config_revision":27}`
